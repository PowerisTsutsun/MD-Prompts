...
>
> **Set reasoning effort to high.** Phase 0's baseline is a contract the later phases are
> checked against; a model that forgets it will delete something live and call it dead.
>
> **No shell or file access** (e.g. ChatGPT web): you cannot establish the Phase 0 baseline,
> and without it every deletion is a guess. Do not propose removals — instead list what you
> would check and the exact commands to run, and mark all of it **UNVERIFIED**.
>
> Read `AGENTS.md` / `CLAUDE.md` / `.cursor/rules/` / `CONTRIBUTING.md` if present and
