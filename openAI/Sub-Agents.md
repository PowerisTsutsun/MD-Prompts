# General Use Prompt — Exhaustive Full-Project Review Roles

Stack-agnostic **and harness-agnostic**: OpenAI Codex CLI, GPT-5-class models via
the API, Cursor, Windsurf, Aider, Copilot Agent, Gemini CLI, or plain ChatGPT with
a role block pasted in.

Fifteen review roles follow. Drop this whole file in as reference, or split each
`---` block into its own file and load it per your harness (see **Installing**
below).

**Mode: EXHAUSTIVE.** Every role defaults to full-project scope. No sampling,
...
Claude Code has a native subagent system that auto-dispatches these. Most other
harnesses don't, so a role is loaded as a **prompt you invoke by name** — the
work is identical, only the dispatch differs.

| Harness | Where each block goes | How you invoke it |
|---|---|---|
...
reach for. `AGENTS.md` is read automatically by Codex CLI, Cursor, Aider, Copilot
Agent and Gemini CLI — it's the cross-vendor equivalent of `CLAUDE.md`.

**Set reasoning effort to high.** These roles demand sustained whole-repo coverage;
low-effort settings quietly turn exhaustive passes into spot checks.

### Degraded mode (no shell, no file access)

Roles that must enumerate files, run greps, or execute a test suite cannot verify
anything in a chat-only harness. In that setting the model must:

1. State which steps it cannot perform, up front.
2. Ask for the specific files it needs, in risk order per the routing table.
3. Label every finding **UNVERIFIED** and give the exact command the user should
   run to confirm it.

Never let a chat-only session emit a report that reads as if the repo was swept.

---

## How this works

The **main agent** (you, or the orchestrating session) delegates to the matching
role below, gets a structured report back, and decides what to do with it.

If your harness supports parallel agents or multiple sessions, run independent
roles concurrently — but every finding still gets verified against real code by
the orchestrator before it enters a final report.
...
capabilities: read files, search (grep/glob), shell — read-only, no writes
reasoning: high   # most capable model available; low effort breaks the coverage protocol
---
```
...
capabilities: read files, search (grep/glob), shell, write/edit files
reasoning: high   # most capable model available; low effort breaks the coverage protocol
---
```
...
capabilities: read files, search (grep/glob), shell, web search + fetch
reasoning: high   # most capable model available; low effort breaks the coverage protocol
---
```
