...
5. Read `AGENTS.md` / `CLAUDE.md` / `.cursor/rules/` / `CONTRIBUTING.md` if present and respect project conventions.
...
**For large codebases:** if your harness can dispatch parallel agents or you can run several sessions side by side, split the manifest into independent directory chunks and audit them concurrently. Each worker writes findings in the same format to its own findings file; the orchestrator merges them into `findings.md` and must verify every imported finding by reading the cited code itself before it enters the report. Coverage checkoffs in `manifest.md` happen only after verification. Where parallel workers aren't available, work the manifest serially in risk order — the `.bug-hunt/` state files exist precisely so a single session can survive context loss and resume.
