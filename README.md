...
| [`session-logger.md`](session-logger.md) | 119 | Work Session Logger agent — tracks meaningful work during a session and emits a daily summary plus an employer-friendly timesheet, with explicit rules on what does and doesn't count as loggable. |
| [`docs-maintainer.md`](docs-maintainer.md) | 110 | Project Documentation Maintainer agent — keeps docs accurate after changes, defines the minimum doc set, changelog behavior, and a truthfulness rule that forbids documenting what wasn't verified. |
